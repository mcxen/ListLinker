...
import 'package:list_linker/database/alist_database.dart';
import 'package:list_linker/database/dao/favorite_dao.dart';
import 'package:list_linker/database/dao/file_download_record_dao.dart';
import 'package:list_linker/database/dao/file_password_dao.dart';
import 'package:list_linker/database/dao/file_viewing_record_dao.dart';
import 'package:list_linker/database/dao/server_dao.dart';
import 'package:list_linker/database/dao/video_viewing_record_dao.dart';
...
class AlistDatabaseController extends GetxController {
  ...
  Future<void> init() async {
    var dbName = "listlinker.db";
    if (Platform.isIOS) {
      var directory = await getApplicationSupportDirectory();
      var dbPath = path.join(directory.path, "database", "listlinker.db");
      if (!await File(dbPath).exists()) {
        var directoryOld = await getApplicationDocumentsDirectory();
        var dbPathOld = path.join(directoryOld.path, "listlinker.db");
